...
public class CarroService {
    ...
    public List<Carro> search(CriteriaRequest criteriaRequest){
        ...
        if(criteriaRequest.modelo().isPresent()){
            predicates.add(cb.like(cb.lower(carro.get("modelo")), "%"+criteriaRequest.modelo().get().toLowerCase()+"%"));
        }
        if(criteriaRequest.fabricante().isPresent()){
            predicates.add(cb.like(cb.lower(carro.get("fabricante")), "%"+criteriaRequest.fabricante().get().toLowerCase()+"%"));
        }
        if(criteriaRequest.pais().isPresent()){
            predicates.add(cb.like(cb.lower(carro.get("pais")), "%"+criteriaRequest.pais().get().toLowerCase()+"%"));
        }
